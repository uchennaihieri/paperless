...
const forms = [
  ...
  },
  {
    ...
    fields: [
      ...
      { id: "f8", label: "Attach Invoice or Other documentation showing validity of request", type: "file", required: true, description: "File number limit: 10, Single file size limit: 100MB, Allowed file types: Word, Excel, PPT, PDF, Image, Video, Audio", maxFiles: 10, accept: ".doc,.docx,.xls,.xlsx,.ppt,.pptx,.pdf,image/*,video/*,audio/*" },
    ]
  },
  {
  ...
  },
  {
    ...
    fields: [
      ...
      { id: "f10", label: "Attach Expense Items Excel and Receipts", type: "file", required: true, description: "File number limit: 10, Single file size limit: 100MB, Allowed file types: Excel, PDF, Image", maxFiles: 10, accept: ".xls,.xlsx,.pdf,image/*" },
    ]
  },
  {
    name: "EXPENSE REIMBURSABLE FORM",
    description: "Request repayment for out-of-pocket expenses already paid while carrying out official duties.",
    fields: [
      ...
      { id: "f11", label: "Attach Expense Items on Excel Sheet or Receipts", type: "file", required: true, description: "File number limit: 10, Single file size limit: 100MB, Allowed file types: Word, Excel, PPT, PDF, Image", maxFiles: 10, accept: ".doc,.docx,.xls,.xlsx,.ppt,.pptx,.pdf,image/*" },
    ]
  },
  {
...
async function main() {
  for (const form of forms) {
    ...
    const data = {
      ...form,
      formOwner: "Head Office",
      formTreater: "Head Office",
