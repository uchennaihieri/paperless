...
export async function createFormTemplate(name: string, description: string, fields: any[], formOwner?: string, formTreater?: string, formTreaterFallbackBranch?: string, formTreaterRole?: string, pdfTemplateId?: string, mobileEnabled: boolean = false, accountServicesEnabled: boolean = false, isInternal: boolean = false, needsContract: boolean = false, contractTemplateId?: string, automatedSignatories: any[] = [], automatedSigningType: string = "sequential", generatesExcel: boolean = false, pdfGeneratorType?: string, templateMappings?: Record<string, string>, conditionalRouting?: any) {
  const result = await apiClient("/forms", {
    method: "POST",
    body: JSON.stringify({ name, description, fields, formOwner, formTreater, formTreaterFallbackBranch, formTreaterRole, pdfTemplateId, mobileEnabled, accountServicesEnabled, isInternal, needsContract, contractTemplateId, automatedSignatories, automatedSigningType, generatesExcel, pdfGeneratorType, templateMappings, conditionalRouting })
  });
