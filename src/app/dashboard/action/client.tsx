...
export default function ActionClient({ items, viewMode = "list", detailId }: { items: ActionItem[], viewMode?: "list" | "detail", detailId?: string }) {
  ...
  useEffect(() => {
    if (statusMode !== "complete" && statusMode !== "delegate") return;
    const run = async () => {
      if (searchQuery.length < 1) {
        setSearchResults(statusMode === "complete" ? [{ _none: true }] : []);
        return;
      }
      const res = await searchActiveWorkflowUsers(searchQuery);
      setSearchResults(statusMode === "complete" ? [{ _none: true }, ...res] : res);
    };
  ...
  const handleDelegateProcess = async () => {
    setIsChanging(true);
    try {
      const email = selectedApprover?.finca_email;
      if (!email) {
        setAssignError("Please select a user to delegate to.");
        setIsChanging(false);
        return;
      }
            ...
            <div className="p-6">
              ...
              {!statusMode && (
                <div className="flex flex-col gap-3">
                  ...
                  {(!selected.status.startsWith("Assigned") ||
                    (currentUserEmail && selected.treaterEmail?.toLowerCase() === currentUserEmail)) && (
                      <Button variant="outline" className="h-14 justify-start px-6 cursor-pointer hover:bg-blue-50 hover:border-blue-300" onClick={() => setStatusMode("delegate")}>
              ...
              {statusMode === "delegate" && (
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
                  <p className="text-sm text-gray-600">Search for a user to delegate this form to.</p>
                      ...
                      {searchResults.map((u, idx) =>
                        u._none ? null : (
                          ...
                          >
                            ...
                            {u.user_role && <div className="text-[10px] opacity-60 mt-0.5">{u.user_role} • {u.branch}</div>}
                          </button>
                        )
                      )}
                    </div>
                  ...
                  <div className="pt-4 flex gap-3">
                    <Button variant="outline" className="flex-1 cursor-pointer" onClick={() => { setStatusMode(""); setSearchQuery(""); setSearchResults([]); }}>Cancel</Button>
                    <Button
                      className="flex-1 cursor-pointer bg-blue-600 hover:bg-blue-700 text-white"
                      disabled={!selectedApprover || isChanging || selectedApprover._none}
                      onClick={handleDelegateProcess}
                    >
                      {isChanging ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : "Delegate Form"}
                    </Button>
                  </div>
                </div>
              )}
